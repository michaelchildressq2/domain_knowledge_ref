...
aliases:
  ...
  - apply from start
applies_when:
  - The user asks about apply from the start in an Infrastructure as Code or platform engineering context.
...
sources:
  - "book: Kief Morris, Infrastructure as Code, second edition, iac2.pdf; Chapter 8."
...
When a pipeline fails, fix source and rerun from the beginning instead of patching later stages by hand.
...
Use apply from the start as a deliberate design choice. Make the boundary, ownership, inputs, outputs, and failure behavior visible in code and delivery workflow. Prefer small, testable, reviewable changes over broad implicit behavior.
...
Synthesized from Chapter 8. Source confidence is high because the chapter text was extracted locally from the user-provided PDF. No source excerpts are stored here.
