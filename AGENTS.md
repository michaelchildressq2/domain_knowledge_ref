...
- **One task per invocation.** When working from GitHub Issues, complete exactly one issue-sized task, commit, update the issue with the result, and stop. Never batch multiple tasks.
...
Issues for this repo are created in GitHub using the `gh` CLI. See `docs/agents/issue-tracker.md` before creating, reading, labeling, commenting on, or closing issues.
